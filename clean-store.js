...
const MASTER={Vial:'',Pen:'',Cartridge:'/assets/cartridge-master-v5.svg?masterv=5'};
...
function imageFor(v){return MASTER[v?.format]||v?.image_url||''}
...
async function loadData(){
 ...
 if(!mr.error)(mr.data||[]).forEach(m=>{if(m?.format&&m?.master_image_url&&m.format!=='Cartridge')MASTER[m.format]=m.master_image_url+(m.master_image_url.includes('?')?'&':'?')+'masterv='+encodeURIComponent(m.version||1)});
 MASTER.Cartridge='/assets/cartridge-master-v5.svg?masterv=5';
...
function renderHeroAssets(){const map=[['heroVial','Vial'],['heroPen','Pen'],['heroCartridge','Cartridge']];for(const [id,f] of map){const e=document.getElementById(id);if(e&&MASTER[f])e.src=MASTER[f]}}
...
function productCard(p){
 ...
 <div class="product-media"><span class="category-badge">${esc(p.categories?.name||'Research')}</span><span class="format-badge">${esc(v?.format||'—')}</span>${image?`<img src="${esc(image)}" alt="${esc(p.name+' '+(v?.strength_label||'')+' '+(v?.format||''))}" loading="lazy">`:''}<div class="dynamic-label"><div class="name">${esc(p.name)}</div><div class="strength">${esc(v?.strength_label||'')}</div></div></div>
