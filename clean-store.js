...
const MASTER={
 Vial:'https://yjauxyvtrmdriwtmckkl.supabase.co/storage/v1/object/public/catalog-media/masters/vial-master?v=1787940592891',
 Pen:'https://yjauxyvtrmdriwtmckkl.supabase.co/storage/v1/object/public/catalog-media/masters/pen-master?v=1787940669584',
 Cartridge:'https://yjauxyvtrmdriwtmckkl.supabase.co/storage/v1/object/public/catalog-media/masters/cartridge-master?v=1787940486688'
};
...
function renderHeroAssets(){const map=[['heroVial','Vial'],['heroPen','Pen'],['heroCartridge','Cartridge']];for(const [id,f] of map){const e=document.getElementById(id);if(e)e.src=MASTER[f]}}
...
function productCard(p){
 ...
 const cat=categoryColor(p), ok=available(v), image=imageFor(v);
 return `<article class="product-card" id="card-${esc(p.id)}" data-format="${esc(v?.format||'Vial')}" style="--cat:${cat}">
 <div class="product-media"><span class="category-badge">${esc(p.categories?.name||'Research')}</span><span class="format-badge">${esc(v?.format||'—')}</span><img src="${esc(image)}" alt="${esc(p.name+' '+(v?.strength_label||'')+' '+(v?.format||''))}" loading="lazy"><div class="dynamic-label"><div class="name">${esc(p.name)}</div><div class="strength">${esc(v?.strength_label||'')}</div></div></div>
...
window.openStageAccount=()=>modal('Member Area — Staging',`<p>The production member area is intentionally isolated from this preview.</p><div class="meta-box"><small>Status</small><b>Protected until isolated staging auth, orders, wallet and protocol entitlements are configured.</b></div><p>This button is working correctly: it does not route you into production by accident.</p>`);
window.stageCheckout=()=>modal('Checkout — Staging Safety Gate',`<p>Your cart is ready for UI testing, but order/payment writes are intentionally disabled in this staging environment.</p><div class="meta-box"><small>Current cart</small><b>${cart.reduce((n,x)=>n+x.qty,0)} item(s) · ${money(cart.reduce((s,x)=>s+x.price*x.qty,0))}</b></div><p>Checkout will be enabled after the staging order, wallet, ToyyibPay and shipping services are isolated and tested.</p>`);
