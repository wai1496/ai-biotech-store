...
(()=>{
  ...
  function panel(title){
    ...
    const close=document.createElement('button');close.textContent='Close';close.className='btn';close.onclick=()=>{panelRevision++;host.hidden=true;};host.appendChild(close);
  ...
  window.openPublishedPage=async slug=>{
    ...
    if(revision!==panelRevision||host.hidden)return;
  ...
  document.addEventListener('keydown',e=>{if(e.key==='Escape'){panelRevision++;const host=document.getElementById('sharedContent');if(host)host.hidden=true;window.closeGuides?.();}});
})();
