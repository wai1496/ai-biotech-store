...
(()=>{
  ...
  const operationalRoutes=new Set(['/member.html','/checkout.html','/payment-return.html','/admin.html']);
  const writesEnabled=valid&&operationalRoutes.has(String(window.location?.pathname||''))&&cfg.checkoutEnabled===true&&cfg.memberEnabled===true;
  ...
  window.AIBTRuntime=Object.freeze({createClient,valid,writesEnabled,reason:writesEnabled?'Authenticated Preview access against isolated Staging only.':error.message,scope:'preview-rpnwssqvurpdennpzplx'});
