...
(function(){
  ...
  function renderThumbs(p){
    ...
    if(!host)return;
    host.innerHTML=(p.forms||[]).map(form=>`<button class="pd-thumb${mf.value===form?' active':''}" data-pd-thumb="${eh(form)}" onclick="selectPDForm(decodeURIComponent('${enc(form)}'))"><div class="visual" data-pd-thumb-vis="${eh(form)}"></div><small>${eh(form)}</small></button>`).join('');
      ...
      <div class="pd-wrap">
        ...
        <div class="pd-main">
          ...
          <div class="pd-info">
            ...
            <div class="pd-options" id="pdStrengths">${(p.strengths||[]).map(s=>`<button class="pd-choice ${s===initial.strength?'active':''}" data-strength="${eh(s)}" onclick="selectPDStrength(this)">${eh(main(s))}</button>`).join('')}</div>
            <span class="pd-label">Format</span>
            <div class="pd-options" id="pdForms">${(p.forms||[]).map(f=>`<button class="pd-choice ${f===initial.form?'active':''}" data-form="${eh(f)}" onclick="selectPDForm(decodeURIComponent('${enc(f)}'))">${eh(f)}</button>`).join('')}</div>
            <input type="hidden" id="ms" value="${eh(initial.strength)}"><input type="hidden" id="mf" value="${eh(initial.form)}">
            <div id="vinfo"></div>
            <div class="pd-buy"><div class="pd-qty"><button type="button" onclick="changePDQty(-1)">−</button><span id="pdQtyEl">1</span><button type="button" onclick="changePDQty(1)">+</button></div><button id="addBtn" class="pd-cart" onclick="addCartPD()">ADD TO CART</button><button class="pd-heart" type="button" aria-label="Save product">♡</button></div>
            <div class="pd-perks"><span>▣ Fresh made same day ship</span><span>❄ Keep refrigerated</span><span>⚗ Variant-specific stock</span></div>
          </div>
        </div>
        <div class="pd-related"><h3>RELATED PRODUCTS</h3><div class="pd-related-grid">${related.map((r,i)=>`<button class="pd-related-card" style="--rc:${color(r)}" onclick="openProduct(decodeURIComponent('${enc(r.id)}'))"><div class="rvis" id="rvis${i}"></div><b>${eh(r.name)}</b><small>${eh(main(r.strengths?.[0]||''))} / ${eh(r.forms?.[0]||'Vial')}</small><span class="rprice">${minPrice(r)?'RM'+minPrice(r).toFixed(2):'Unavailable'}</span></button>`).join('')}</div></div>
      </div>`;
    window.pdQty=1;
