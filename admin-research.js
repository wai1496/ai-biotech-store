(function(){
  ...
  function reviewVersion(versionId){
    ...
    const host=modal(`<div class="ar-review-head"><div><h2>${editable?'Review Draft':'Research Version'} · v${safe(version.version_number)}</h2><p class="muted">${safe(version.provider)}${version.model?` / ${safe(version.model)}`:''} · ${safe(statusLabel(version.status))}</p></div>${evidenceBadge(gate)}</div>
      <section class="ar-evidence ar-review-section"><h3>Evidence Gate</h3><div class="ar-evidence-grid"><div><span>High-quality sources</span><b>${Number(gate.high_quality_source_count||0)}</b></div><div><span>Unique domains</span><b>${Number(gate.unique_domain_count||0)}</b></div><div><span>Exact-product evidence</span><b>${safe(gate.exact_product_evidence||'none')}</b></div><div><span>Human evidence</span><b>${safe(gate.human_evidence||'none')}</b></div><div><span>Preclinical evidence</span><b>${safe(gate.preclinical_evidence||'not identified')}</b></div><div><span>Contradictions</span><b>${gate.contradictions_found?'Found':'Not flagged'}</b></div></div>${!gate.passed?'<div class="ar-gate-warning">Evidence gate did not auto-pass. You may continue only after manually reviewing the warnings and sources.</div>':''}</section>
      <details class="ar-review-section" open><summary class="ar-review-summary">Warnings</summary><div class="ar-review-content">${warnings.length?`<ul class="ar-warnings">${warnings.map(w=>`<li>${safe(w)}</li>`).join('')}</ul>`:'<p class="muted">No automated warnings.</p>'}</div></details>
      <details class="ar-review-section" open><summary class="ar-review-summary">Sources</summary><div class="ar-review-content">${sourceList(version.sources_json)}</div></details>
      <details class="ar-review-section ar-comparison" open><summary class="ar-review-summary">Published vs Draft</summary><div class="ar-review-content"><div class="ar-compare">${compareColumn('Published',published,changed,false)}${compareColumn('Draft',version.profile_json||{},changed,true)}</div></div></details>
      ${editable?`<section class="ar-approval"><label>Review / verification note${!gate.passed?' (required)':''}<textarea name="verification_note" rows="3" placeholder="Record what you checked before publishing."></textarea></label><div class="ar-form-actions ar-approval-actions"><button type="button" class="btn primary" data-ar-publish>Approve &amp; Publish</button><button type="button" class="btn" data-ar-reject>Reject</button></div></section>`:'<div class="notice">This published/history version is read-only.</div>'}`);
