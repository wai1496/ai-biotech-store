...
async function insertPendingPayment(order,billCode){
  const body={order_id:order.id,user_id:order.user_id,gateway:GATEWAY,amount:Number(order.grand_total),currency:order.currency||'MYR',status:'pending',gateway_reference:billCode,metadata:{mode:'sandbox',bill_code:billCode,order_number:order.order_number||null,duitnow_qr:true}};
...
module.exports={GATEWAY,requireSandboxConfig,getAuthedUser,getOwnedOrder,serviceRest,publicOrigin,checkDuitNowQRStatus,createSandboxBill,findPendingPayment,insertPendingPayment,getBillTransactions,mapTransactionStatus,persistVerifiedTransaction,callbackHash,safeEqualHex,parseBody};
