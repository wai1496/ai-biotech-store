...
function previewSafety(){
  ...
  if(evidence.status!=='approved'){const e=new Error('Preview writes and provider calls are locked pending isolated database contract evidence.');e.status=503;e.code='PREVIEW_DATABASE_CONTRACT_UNVERIFIED';throw e;}
  return {preview:true,supabaseUrl:runtime.url};
}
