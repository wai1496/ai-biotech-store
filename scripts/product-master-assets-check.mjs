...
for (const file of requiredAssets) assert.equal(exists(file), true, `${file} must exist for WP-03`);

assert.equal(/master-pending/i.test(config), false, 'staging-config.js must not use pending masters');
...
for (const file of ['assets/vial-master-v4.svg','assets/pen-master-v4.svg','assets/cartridge-master-v5.svg']) {
  ...
  assert.match(svg, /data:image\/(?:webp|png);base64,/i, `${file} must embed approved artwork`);
...
console.log('PASS: final fixed-hardware Vial / printed Pen / no-overlay Cartridge contract');
