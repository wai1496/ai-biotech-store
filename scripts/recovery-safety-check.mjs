...
if(!fail.length){
 ...
 for(const token of ['ops_create_recovery_snapshot','ops_compare_recovery_snapshot','ops_preview_recovery_restore','ops_restore_recovery_module','p_expected_current_hash','RESTORE ${module}','Non-destructive upsert','Automatic pre-restore safety snapshot'])if(!ui.includes(token))fail.push(`Recovery UI missing safety contract token: ${token}`);
 const destructive=[/\.from\([^)]*\)\.delete\s*\(/i,/\.rpc\([^)]*(reset|truncate|drop|delete)/i,/\btruncate\s+table\b/i,/\bdrop\s+table\b/i,/\bdelete\s+from\b/i];
 for(const re of destructive)if(re.test(ui))fail.push(`Recovery UI contains destructive database/reset pattern: ${re}`);
 ...
 if(!ui.includes('There is deliberately <b>no “Reset Website” button</b>'))fail.push('Recovery UI must explicitly disclose that no one-click reset exists');
 ...
 if(/\b(prompt|confirm|alert)\s*\(/.test(ui))fail.push('Recovery UI contains forbidden native browser dialog');
