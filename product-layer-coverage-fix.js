...
const KNOWN_MISSING_STRENGTHS=Object.freeze(new Set([
 'blue:3ml','blue:10ml','green:10mg','yellow:5mg','red:20mg'
]));
...
function colorTokenFromSrc(src){
 const m=String(src||'').match(/-(orange|green|red|purple|pink|gold|yellow|light-blue|blue)\.webp/i);
 return m?m[1].toLowerCase():'';
}
function colorFromSrc(src){
 const token=colorTokenFromSrc(src);
 return token?FALLBACK_HEX[token]:'#1477ff';
}
function isKnownMissingStrength(src,strengthSlug){
 const token=colorTokenFromSrc(src);
 return !!token&&KNOWN_MISSING_STRENGTHS.has(`${token}:${strengthSlug}`);
}
function renderFallback(img,kind,text){
...
function addFallback(img,kind,text){
 ...
 img.addEventListener('error',()=>renderFallback(img,kind,text));
}
function repairLayer(img){
 ...
 if(kind==='name'){
   ...
   const nextSrc=src.replace(/(aibiotech-(?:vial|pen)-strength-)[^-]+(-(?:orange|green|red|purple|pink|gold|yellow|light-blue|blue)\.webp(?:\?.*)?)$/i,`$1${slug}$2`);
   if(isKnownMissingStrength(nextSrc,slug)){
     img.src=nextSrc;
     renderFallback(img,'strength',text);
     return;
   }
   img.src=nextSrc;
...
window.AIBT_LAYER_COVERAGE_FIX=Object.freeze({slugName,slugStrength,repair,colorFromSrc,isKnownMissingStrength});
})();
