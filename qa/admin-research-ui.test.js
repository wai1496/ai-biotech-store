...
assert.match(js,/supabaseUrl/,'research admin must inspect its active Supabase target before allowing writes');
assert.match(js,/Preview is connected to production data/i,'preview must block research writes when pointed at production Supabase');
assert.match(js,/git-(?:feature|review|staging)-/i,'preview/staging host detection must be explicit');
