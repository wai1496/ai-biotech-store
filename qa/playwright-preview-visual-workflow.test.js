...
assert(workflow.includes('actions/github-script@v7'), 'workflow must request a short-lived GitHub OIDC token');
assert(workflow.includes('core.getIDToken()'), 'workflow must use GitHub OIDC without a persistent credential');
assert(workflow.includes("VERCEL_TRUSTED_OIDC_TOKEN: ${{ steps.oidc.outputs.token }}"), 'OIDC token must only enter the visual-QA runner step');
...
assert(runner.includes("requestUrl.hostname === previewUrl.hostname"), 'OIDC token must only be sent to the Preview origin');
