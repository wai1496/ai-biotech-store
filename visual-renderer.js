...
(function(global){
  ...
  const CARTRIDGE_FIELDS={
    name:{x:731,y:762,w:76,h:198,pad:10,max:28,min:12,weight:900,vertical:true},
    strength:{x:713,y:991,w:110,h:52,pad:8,max:24,min:12,weight:900}
  };
  /* Label masks deliberately avoid fixed hardware. */
  ...
  const CARTRIDGE_LABEL_MASKS=[{x:700,y:740,w:200,h:390}];
  ...
  function splitBalanced(value,threshold=22,maxPart=24){
    const text=String(value||'').trim();if(!text||text.length<=threshold)return text?[text]:[];
    ...
    for(let i=1;i<words.length;i++){
      ...
      if(a.length<=maxPart&&b.length<=maxPart&&diff<bestDiff){best=[a,b];bestDiff=diff}
    }
    if(best.length===2)return best;
    const pivot=Math.max(1,Math.min(words.length-1,Math.round(words.length/2)));
    return [words.slice(0,pivot).join(' '),words.slice(pivot).join(' ')];
  }
  function splitVialName(value){return splitBalanced(value,22,24)}
  function splitCartridgeName(value){return splitBalanced(value,18,22)}
  ...
  function recolorCartridgeAccents(ctx,accent){for(const rect of CARTRIDGE_LABEL_MASKS)recolorRegion(ctx,accent,rect,isOrange)}
  ...
  function printVialName(ctx,text,field,fill){
    const lines=splitVialName(text);if(lines.length<=1){printCenteredField(ctx,text,field,fill);return}
    const twoLineField={...field,max:34,min:18},lineGap=35;
    lines.slice(0,2).forEach((line,index)=>{const layout=fitTextLayout((t,s)=>measureWidth(ctx,t,s,twoLineField.weight),line,twoLineField),y=field.cy+(index===0?-lineGap/2:lineGap/2);ctx.save();ctx.font=`${twoLineField.weight} ${layout.size}px Arial`;ctx.textBaseline='middle';ctx.fillStyle=fill;ctx.textAlign='center';ctx.translate(field.cx,y);ctx.scale(layout.scaleX,1);ctx.fillText(line,0,0);ctx.restore()});
  }
  function printCartridgeName(ctx,text,field,fill){
    const lines=splitCartridgeName(text),cx=field.x+field.w/2,cy=field.y+field.h/2;
    const fitField={w:field.h,h:field.w,pad:field.pad,max:field.max,min:field.min,weight:field.weight};
    ctx.save();ctx.beginPath();ctx.rect(field.x,field.y,field.w,field.h);ctx.clip();ctx.translate(cx,cy);ctx.rotate(-Math.PI/2);ctx.textBaseline='middle';ctx.textAlign='center';ctx.fillStyle=fill;
    const gap=20;
    lines.slice(0,2).forEach((line,index)=>{const layout=fitTextLayout((t,s)=>measureWidth(ctx,t,s,fitField.weight),line,fitField);ctx.save();ctx.translate(0,lines.length>1?(index===0?-gap/2:gap/2):0);ctx.font=`${fitField.weight} ${layout.size}px Arial`;ctx.scale(layout.scaleX,1);ctx.fillText(line,0,0);ctx.restore()});
    ctx.restore();
  }
  ...
  async function renderPreview({canvas,masterUrl,productName,strength,format,accent='#18c9ff',cartridgeBlank=false,vialCapMode='white'}){
    ...
    if(form==='Vial'){recolorLabelAccents(ctx,accent);if(capMode==='category')recolorVialCap(ctx,accent);printVialName(ctx,name,VIAL_FIELDS.name,nameColor);printCenteredField(ctx,dose,VIAL_FIELDS.strength,'#111');return {mode:'dynamic-preview',format:form,capMode}}
    if(form==='Cartridge'&&cartridgeBlank){recolorCartridgeAccents(ctx,accent);printCartridgeName(ctx,name,CARTRIDGE_FIELDS.name,nameColor);printField(ctx,dose,CARTRIDGE_FIELDS.strength,'#111');return {mode:'dynamic-preview',format:form}}
    throw new Error(`Unsupported format: ${form}`)
  }
  global.AIBTVisualRenderer={SIZE,PEN_FIELDS,VIAL_FIELDS,CARTRIDGE_FIELDS,VIAL_LABEL_MASKS,VIAL_CAP_REGION,VIAL_CAP_MASK,CARTRIDGE_LABEL_MASKS,normalizeLabel,normalizeStrength,fitFontSize,fitTextLayout,splitVialName,splitCartridgeName,readableAccent,recolorLabelAccents,recolorCartridgeAccents,recolorVialCap,renderPreview};
