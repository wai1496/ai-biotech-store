...
module.exports=async function handler(req,res){
  ...
  try{
    const upstream=await fetch('https://api.openai.com/v1/responses',{
      ...
      body:JSON.stringify({model:process.env.AIBT_PROGRESS_CHAT_MODEL||'gpt-5.6-luna',input:[{role:'system',content:[{type:'input_text',text:'You are the AI BioTech engineering project status assistant. Answer only about the supplied project status. Be concise, operational, and explicit about unknowns. Never claim a fix, deployment, test, or credential exists unless shown in the status. Never request or reveal secret values. If the user asks to make changes, explain what is currently known and state that this read-only control center cannot execute production mutations.'}]},{role:'user',content:[{type:'input_text',text:`Current project status JSON:\n${JSON.stringify(compact)}\n\nQuestion: ${message}`}]}],max_output_tokens:500})
    });
    const data=await upstream.json().catch(()=>({}));
    if(!upstream.ok){console.error('Progress chat upstream error',upstream.status,data?.error?.type||data?.error?.code||'unknown');res.status(200).json({answer:'The AI project assistant is temporarily unavailable. The live status dashboard remains available.',mode:'ai_unavailable',generatedAt:new Date().toISOString()});return;}
    ...
    const timedOut=error?.name==='AbortError';console.error('Progress chat request failed',timedOut?'timeout':error?.message||error);res.status(200).json({answer:timedOut?'The AI project assistant timed out. Please use the live dashboard status for now.':'The AI project assistant is temporarily unavailable. The live dashboard remains available.',mode:'ai_unavailable',generatedAt:new Date().toISOString()});
