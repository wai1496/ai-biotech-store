...
async function authFetch(path,{method='GET',body}={}){
  ...
  if(!response.ok){const error=new Error(String(data?.msg||data?.message||data?.error_description||'Supabase admin request failed.'));error.status=response.status;throw error;}
  return data;
}
module.exports=async function handler(req,res){
  try{
    ...
    console.error('Preview QA user bootstrap failed',error?.status||error?.message||error);
