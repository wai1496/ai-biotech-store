...
async function refreshAgentAccess(){
  ...
  const {data,error}=await db.rpc('member_get_agent_dashboard');if(error)return;
  agentData=data||null;const btn=$('#agentTabBtn');if(btn)btn.hidden=!agentData?.is_agent;
}
...
window.openAgentDashboard=async()=>{
  ...
  panel.innerHTML=`<div class="agent-head"><div><h2>Agent Dashboard</h2><p class="muted">Role pricing and referral visibility are live in staging. Commission creation remains disabled until the commission base/timing rule is approved.</p></div>${badge(ap.status||agentData.agent_status)}</div>
...
window.addEventListener('DOMContentLoaded',()=>{setTimeout(refreshAgentAccess,200)});
window.addEventListener('focus',refreshAgentAccess);
})();